/**
 * Frontend mirror of backend deriveFatadeMeasurements (facade-measurements.util.ts).
 * Diagnostic-only (no plan2d) — closes the live-preview gap where derived
 * labor/volume keys (facadeAreaLabor, insulationVolumeM3, meshArea, paintingArea…)
 * were all 0 because the derivation was never run client-side.
 */
...
export function deriveFatadeMeasurements(
  ...
  m.heightMultiplier = resolveFacadeHeightMultiplier(buildingHeightM);
