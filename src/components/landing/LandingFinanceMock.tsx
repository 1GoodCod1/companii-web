...
const invoices = [
  { nr: 'FAC-2026-089', client: 'Ion Popescu', amount: '4.850', status: 'Plătită', tone: 'text-emerald-600 bg-emerald-50' },
  { nr: 'FAC-2026-088', client: 'SC Voltex SRL', amount: '12.400', status: 'Restantă', tone: 'text-amber-600 bg-amber-50' },
  { nr: 'FAC-2026-087', client: 'Maria Rusu', amount: '2.100', status: 'Plătită', tone: 'text-emerald-600 bg-emerald-50' },
];
    ...
    >
      ...
      <div className="rounded-[1.75rem] border border-white/60 bg-white/95 shadow-[0_40px_100px_-20px_rgba(16,185,129,0.2)] overflow-hidden backdrop-blur-xl">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100/80 bg-slate-50/80">
          ...
          <span className="ml-3 text-[10px] font-medium text-gray-400">app.fabercompanii.md/company/facturi</span>
        </div>
        ...
        <div className="p-5 sm:p-6 grid lg:grid-cols-5 gap-5 bg-gradient-to-br from-white via-emerald-50/20 to-indigo-50/10">
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-black text-gray-900 mt-1">84.200 MDL</p>
          ...
          <div className="lg:col-span-3 rounded-2xl bg-white/70 border border-gray-100/80 p-4">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-bold text-gray-900">Facturi recente</p>
              ...
              {invoices.map((inv) => (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-[10px] text-gray-400 font-medium">{inv.nr}</p>
                    <p className="text-xs font-semibold text-gray-800 truncate">{inv.client}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-bold text-gray-900">{inv.amount} MDL</p>
                    <span className={`inline-block mt-0.5 rounded-full px-2 py-0.5 text-[9px] font-semibold ${inv.tone}`}>
                      {inv.status}
                    </span>
                  </div>
                </div>
      ...
      >
        <p className="text-[10px] text-gray-400">TVA calculat automat</p>
        <p className="text-sm font-bold text-gray-800">20% · 970 MDL</p>
      </motion.div>
    </motion.div>
  );
}
